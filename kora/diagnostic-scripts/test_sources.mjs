...
const info = await p.evaluate(() => {
  const rows = [...document.querySelectorAll('.src-row')].map(r => ({
    name: r.querySelector('.name')?.textContent?.trim(),
    sub: r.querySelector('.sub')?.textContent?.trim(),
    chips: [...r.querySelectorAll('.chip')].map(c => c.textContent.trim()),
    html: r.outerHTML.slice(0, 300),
  }));
  return { n: rows.length, rows: rows.slice(0, 14) };
});
console.log('SOURCES', JSON.stringify(info, null, 1));
await b.close().catch(()=>{});
process.exit(0);
